...
public class EnemyAStar : MonoBehaviour {
	...
	public void findUpdatedPath(int currentX,int currentY)
	{
		...
		if (path != null) {
			...
			foreach(GameObject g in GameObject.FindGameObjectsWithTag("GridBox"))
			{
				if(g.renderer.material.color != Color.red && g.renderer.material.color == myColor)
					g.renderer.material.color = Color.white;
			}
			...
			foreach (MyPathNode node in path)
			{
				GameObject.Find(node.X + "," + node.Y).renderer.material.color = myColor;
			}
		}
	...
	public void initializePosition()
	{
		...
		isMoving = false;
		GameObject.Find(startGridPosition.x + "," + startGridPosition.y).renderer.material.color = Color.black; 
